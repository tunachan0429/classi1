...
### 「中断しますか?」などの確認ダイアログが出て止まる場合

テスト系サイトは、次の問題へ移動する（＝ページを離れる）ときに
「中断しますか?」「このページを離れますか?」といった確認ダイアログを出すことがあります。
このツールは**こうしたダイアログを自動で処理して先に進みます**（特別な設定は不要）。

- ページ離脱の確認（beforeunload）は常に承諾して次の問題へ進みます。
- その他の確認は `DIALOG_ACTION` の設定に従います。
  - `accept`（既定）… OK を押して進む
  - `dismiss` … キャンセルを押してそのページに留まる
- もし「OKを押すとテスト自体が中断されてしまう」場合は `DIALOG_ACTION=dismiss` にしてください。
